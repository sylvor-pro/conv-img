...
// Ensure heazyWaveTemplate is exported at the top of the file
...
const cornersTemplate = (params: any) => {
  ...
  // Adjust curve based on balance and velocity
  const balanceFactor = (balance - 50) / 100;
  const velocityFactor = velocity / 100;
  
  // Generate path for each corner
  ...
  for (let i = 0; i < cornerCount; i++) {
    ...
    // If the number of layers reaches the set value, exit loop
...
const interpolateColor = (color1: string, color2: string, factor: number) => {
  // Simple implementation, may need more complex interpolation in real projects
